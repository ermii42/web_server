...
class CarsModel:
    ...
    def insert(self, model, price, power, color, dealer, image):
        """Добавление автомобиля"""
        cursor = self.connection.cursor()
        cursor.execute('''INSERT INTO cars 
                          (model, price, power, color, dealer, image) 
                          VALUES (?,?,?,?,?,?)''',
                       (model, str(price), str(power), color, str(dealer), image))
        cursor.close()
        self.connection.commit()
    ...
    def get_all(self):
        ...
        cursor = self.connection.cursor()
        cursor.execute("SELECT model, price, car_id, image FROM cars")
        rows = cursor.fetchall()
        return rows
    ...
    def get_by_price(self, start_price, end_price):
        ...
        cursor = self.connection.cursor()
        cursor.execute("SELECT model, price, car_id, image FROM cars WHERE price >= ? AND price <= ?", (str(start_price), str(end_price)))
        row = cursor.fetchall()
        return row
    ...
    def get_by_dealer(self, dealer_id):
        ...
        cursor = self.connection.cursor()
        cursor.execute("SELECT model, price, car_id, image FROM cars WHERE dealer = ?", (str(dealer_id)))
        row = cursor.fetchall()
        return row
